<script lang="ts">
  import { noteStrings, state, Tuner } from "../lib/tuner";
  ...
  function init() {
    tuner.start(); // get microphone permission and start
    started = true;
  }
</script>
...
<div class="ac-demo">
        ...
        >
          ...
          {#if state.deviation > 0}
            <div
              ...
              style="height: {(Math.abs(state.deviation) / 50) * 100}%;"
            />
          {:else if state.deviation < 0}
            <div
              class="deviation_low"
              style="height: {(Math.abs(state.deviation) / 50) * 100 - 50}%;"
            />
          {/if}
        </div>
        <p
  ...
  <div class="history">
    {#each tuner.noteHistory as note, i}
      {#if i === tuner.noteHistory.length - 1}
  ...
  <div class="actions">
    ...
    <button class="startBtn" disabled={started} type="button" on:click={init}
      >Start</button
    >
  </div>
  {#if showSettings}
    ...
    >
      <div class="settings-con">
        <div class="setting-sensitevity">
          <p>
            Sensitivity (higher value means lower sensitivity): {tuner.sensitivity}
          </p>
          <input
            type="range"
            bind:value={tuner.sensitivity}
            min="0.01"
            max="0.2"
            step="0.01"
          />
